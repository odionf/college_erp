...
<body style="background-color: #e8f0ff;">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
        ...
        <li class="nav-item">
          <a class="nav-link" href="messages.php">Message Teacher</a>
        </li>
...
<?php
    ...
    if (isset($_POST['submitp'])) {
        ...
        $check =$_POST['olpswd'];
        $result = $mysqli->query("SELECT * FROM users WHERE pid = '$pid' AND parent_pass='$check'");
        if(!$result)
        {
          echo"<script>alert('Invalid Password!')</script>";
        }
        elseif ($r['user_pass']==$_POST['cnpswd']) {
        ...
        else
        {
            if(strlen($_POST['cnpswd'])>=8 && strlen($_POST['cnpswd'])<=20){
              ...
              $qry = "UPDATE `users` SET parent_pass = '$check' WHERE pid='$pid'";
